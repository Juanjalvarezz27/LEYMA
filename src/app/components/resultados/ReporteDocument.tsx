...
export const pdfStyles = StyleSheet.create({
  ...
  subcatTitle: { fontSize: 9.5, fontWeight: 700, paddingVertical: 4, paddingLeft: 5 }, 
  ...
  multiRowDesc: { width: '28%', fontSize: 8, fontWeight: 400, paddingLeft: 15, color: '#334155' }, 
    ...
    <Document>
      <Page size="LETTER" style={pdfStyles.page}>
        ...
        <View style={pdfStyles.topContact}>
          <Text style={pdfStyles.topContactText}>DIRECCIÓN: AVENIDA CORO DETRÁS DEL HOSPITAL, TRUJILLO 3150.</Text>
        ...
        <View style={pdfStyles.patientBox}>
          <View style={pdfStyles.patientColLeft}>
            ...
            <View style={pdfStyles.patientRow}>
              ...
              <Text style={pdfStyles.patientValue}>{calcularEdad(orden.paciente.fechaNacimiento, orden.paciente.esBebe)}</Text>
            </View>
            <View style={pdfStyles.patientRow}>
              <Text style={pdfStyles.patientLabel}>Dirección:</Text>
              <Text style={pdfStyles.patientValue}>{orden.paciente.direccion || 'No registrada'}</Text>
            </View>
            <View style={pdfStyles.patientRow}>
              <Text style={pdfStyles.patientLabel}>Observaciones:</Text>
              <Text style={pdfStyles.patientValue}>{orden.paciente.observaciones || '---'}</Text>
            </View>
          </View>
          ...
          <View style={pdfStyles.patientColRight}>
            <View style={pdfStyles.patientRow}>
              ...
              <Text style={pdfStyles.patientValue}>{orden.paciente.cedula || 'S/N'}</Text>
            </View>
            <View style={pdfStyles.patientRow}>
              <Text style={pdfStyles.patientLabel}>Sexo:</Text>
              <Text style={pdfStyles.patientValue}>{orden.paciente.sexo === 'M' ? 'Masculino' : 'Femenino'}</Text>
            </View>
            <View style={pdfStyles.patientRow}>
              <Text style={pdfStyles.patientLabel}>Teléfono:</Text>
              <Text style={pdfStyles.patientValue}>{orden.paciente.telefono || 'No registrado'}</Text>
            </View>
            <View style={pdfStyles.patientRow}>
              ...
              <View style={{ width: "100%" }}>
                ...
                {Object.entries(catData.subcategorias).map(([subCatNombre, detalles]: [string, any]) => (
                  <View key={subCatNombre} style={{ marginBottom: 10 }}>
                    ...
                    {detalles.length > 1 && detalles.slice(1).map((det: any) => renderDetalleRow(det, subCatNombre))}
                  </View>
                ))}
              </View>
